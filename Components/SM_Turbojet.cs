...
namespace SagsMissiles
{
    public class SM_Turbojet : MissileSecondaryPropeller
    {
        ...
        {
            MaxThrust = Rounding.R0(MissileConstants.BaseThrustLimit * Size.EfficiencyModifier);
            var defaultValue = 1100f * Size.EfficiencyModifier;
            MinThrust = MaxThrust / 10f;
        }
        ...
        public override string Description =>
            _locFile.Get("Desc", "A jet engine, slow acceleration but very efficient.");
        ...
        protected override bool HandleActiveThrust(float thrust)
        {
            try
            {
                var thrust1 = 500f * Size.EfficiencyModifier * ((float)Missile?.Velocity.magnitude / 100f + 1);
                base.HandleActiveThrust(thrust1);
                Missile.ThrusterWantsGravityDisabled = true;
            }
            catch (Exception e)
            {
                AdvLogger.LogException("Unhandled exception in Turbojet.HandleActiveThrust", e,
